...
# Robust build: auto-installs snscrape if missing, then uses Python API (preferred), with subprocess fallback + diagnostics.
...
st.caption("Fetch tweets on a topic and estimate support / oppose / neutral. This build will auto-install snscrape if it's missing.")

with st.expander("Read me"):
    st.markdown("""
**What this does:**  
- Collects tweets via **snscrape** Python API (preferred). If import fails, the app will **auto-install** snscrape and retry.  
...
# ---------------------------
def ensure_snscrape(version: str = "0.7.0.20240619") -> str:
    """
    ...
    # Try to install
    try:
        with st.status("Installing snscrape…", expanded=False):
            cmd = [sys.executable, "-m", "pip", "install", f"snscrape=={version}"]
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
            if proc.returncode != 0:
                return f"pip install failed: {proc.stderr.strip() or proc.stdout.strip()}"
        # Verify import
        try:
            import snscrape  # noqa: F401
            return ""
        except Exception as e2:
            return f"import after install failed: {e2}"
...
if do_run:
    ...
    err = ensure_snscrape()
    if err:
        diag["error"] = f"snscrape auto-install failed: {err}"
